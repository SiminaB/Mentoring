...
Many more approaches exist, some which build on or are similar to regression. For example, [support vector machines (SVMs)](https://en.wikipedia.org/wiki/Support_vector_machine#Regression) for continuous outcomes are equivalent to regression models with a regularization term and the hinge loss instead of the quadratic loss function. Similarly, [neural networks](https://github.com/greenelab/deep-review/blob/master/content/02.intro.md) can be seen as extensions of linear or logistic regression.
