...
We mentioned before that one of the properties that is generally assumed for linear regression is that the error terms are independent and identically distributed. In case this does not hold, as [alterative to OLS](https://en.wikipedia.org/wiki/Generalized_linear_model) is to use either *weighted least squares* (WLS) or *generalized least squares* (GLS) approaches. This means that if the variance-covariance matrix of the error terms is $W$, then instead of minimizing 
$\hat{\beta} = \arg\min||Y-X\beta||$ as before, one would instead want to minimize
$\hat{\beta} = \arg\min(Y-X\beta)'W^{-1}(Y-X\beta)$, known as the *Mahalanobis length.*
The solution to this problem is similar to the OLS, namely:
$\hat{\beta}=(X'W^{-1}X)^{-1}X'W^{-1}Y$. If W has all off-diagonal terms equal to 0 (i.e. the errors are independent), this is the WLS estimate, otherwise it is the GLS estimate.
